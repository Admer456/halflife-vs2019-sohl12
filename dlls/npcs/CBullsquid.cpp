...
#include "game.h"
#include "scripted.h"
#include "projectiles/CSquidSpit.h"
...
int CBullsquid::IRelationship(CBaseEntity* pTarget)
{
	...
	if (gpGlobals->time - m_flLastHurtTime < 5 && FClassnameIs(pTarget->pev, "monster_bigmomma"))
	{
		return R_FR;
	}
...
int CBullsquid::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
	...
	if (!FClassnameIs(pevAttacker, "monster_headcrab") && !FClassnameIs(pevAttacker, "monster_bigmomma"))
	{
...
{
	const int iConditions = IScheduleFlags();
