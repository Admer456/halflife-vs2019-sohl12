...
#include "projectiles/CControllerHeadBall.h"
#include "projectiles/CControllerZapBall.h"
	...
	case CONTROLLER_AE_BALL_SHOOT:
		{
			...
			CControllerHeadBall::Shoot(pev, (m_pCine ? m_hTargetEnt : m_hEnemy), vecStart, Vector(0, 0, 32));
...
void CController::StartTask(Task_t* pTask)
{
	...
	case TASK_GET_PATH_TO_ENEMY_LKP:
		{
			if (BuildNearestRoute(m_vecEnemyLKP, pev->view_ofs, pTask->flData,
	...
	case TASK_GET_PATH_TO_ENEMY:
		{
			...
			if (BuildNearestRoute(pEnemy->pev->origin, pEnemy->pev->view_ofs, pTask->flData,
...
// Intersect 
//=========================================================
Vector CController::Intersect(Vector vecSrc, Vector vecDst, Vector vecMove, float flSpeed)
{
		...
		while (m_flShootTime < m_flShootEnd && m_flShootTime < gpGlobals->time)
		{
			...
			if (m_pCine != nullptr || m_hEnemy != NULL)
			{
				...
				else if (m_hEnemy != NULL)
				{
					...
					vecDir = Intersect(vecSrc, m_hEnemy->BodyTarget(pev->origin), m_vecEstVelocity,
					                   gSkillData.controllerSpeedBall);
				}

				float delta = 0.03490; // +-2 degree
				vecDir = vecDir + Vector(RANDOM_FLOAT(-delta, delta), RANDOM_FLOAT(-delta, delta),
				                         RANDOM_FLOAT(-delta, delta)) * gSkillData.controllerSpeedBall;

				vecSrc = vecSrc + vecDir * (gpGlobals->time - m_flShootTime);

				CControllerZapBall::Shoot(pev, (m_pCine ? m_hTargetEnt : m_hEnemy), vecSrc, vecDir);
			}
			m_flShootTime += 0.2;
		}
...
//	if ( CVAR_GET_FLOAT("stopmove" ) != 0 )
	...
	do
	{
		...
		if (CheckLocalMove(pev->origin, pev->origin + vecDir * flCheckDist, pTargetEnt, &flDist) != LOCALMOVE_VALID)
		{
			...
			CBaseEntity* pBlocker = Instance(gpGlobals->trace_ent);
			if (pBlocker)
			{
				DispatchBlocked(edict(), pBlocker->edict());
			}
			if (pBlocker && m_moveWaitTime > 0 && pBlocker->IsMoving() && !pBlocker->IsPlayer() && (gpGlobals->time -
				m_flMoveWaitFinished) > 3.0)
			{
