...
void CGargantua::StompAttack()
{
	...
	EMIT_SOUND_DYN(edict(), CHAN_WEAPON, pStompSounds[RANDOM_LONG(0, ARRAYSIZE(pStompSounds) - 1)], 1.0, ATTN_GARG, 0,
...
void CGargantua::FlameUpdate()
{
	...
	for (int i = 0; i < 2; i++)
	{
		if (m_pFlame[i])
		{
			...
			{
				CSpiral::StreakSplash(trace.vecEndPos, trace.vecPlaneNormal, 6, 20, 50, 400);
				streaks = TRUE;
			...
			FlameDamage(vecStart, trace.vecEndPos, pev, pev, gSkillData.gargantuaDmgFire, CLASS_ALIEN_MONSTER,
				DMG_BURN);
	...
	float flDamage, int iClassIgnore, int bitsDamageType)
{
	...
	while ((pEntity = UTIL_FindEntityInSphere(pEntity, vecMid, searchRadius)) != nullptr)
	{
		if (pEntity->pev->takedamage != DAMAGE_NO)
		{
			...
			Vector vecSpot = pEntity->BodyTarget(vecMid);
...
// TraceAttack
//=========================================================
void CGargantua::TraceAttack(entvars_t* pevAttacker, float flDamage, Vector vecDir, TraceResult* ptr,
	int bitsDamageType)
{
	...
	if (bitsDamageType & (GARG_DAMAGE | DMG_BLAST))
	{
		...
		{
			EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, pPainSounds[RANDOM_LONG(0, ARRAYSIZE(pPainSounds) - 1)], 1.0, ATTN_GARG,
				0, PITCH_NORM);
...
void CGargantua::DeathEffect()
{
	...
	for (int i = 0; i < 7; i += 2)
	{
		KeyValueData kvd;
		char buf[128];
		const float randomRange = 70;

		position.x += RANDOM_FLOAT(-randomRange, randomRange);
		position.y += RANDOM_FLOAT(-randomRange, randomRange);

		if(i != 0)
			position.z += 15;

		CBaseEntity* pExplosion = Create("env_explosion", position, g_vecZero, nullptr);
		sprintf(buf, "%3d", 60 + (i * 20));
		...
		pExplosion->SetNextThink((i * 0.3));
	}

	CBaseEntity* pSmoker = Create("env_smoker", pev->origin, g_vecZero, nullptr);
...
void CGargantua::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	{
		// HACKHACK!!!
		CBaseEntity* pHurt = GargantuaCheckTraceHullAttack(GARG_ATTACKDIST + 10.0, gSkillData.gargantuaDmgSlash,
			DMG_SLASH);
		if (pHurt)
		{
			...
			EMIT_SOUND_DYN(edict(), CHAN_WEAPON, pAttackHitSounds[RANDOM_LONG(0, ARRAYSIZE(pAttackHitSounds) - 1)],
				1.0, ATTN_NORM, 0, 50 + RANDOM_LONG(0, 15));
		}
		else // Play a random attack miss sound
			EMIT_SOUND_DYN(edict(), CHAN_WEAPON, pAttackMissSounds[RANDOM_LONG(0, ARRAYSIZE(pAttackMissSounds) - 1)],
				1.0, ATTN_NORM, 0, 50 + RANDOM_LONG(0, 15));
	...
	case GARG_AE_LEFT_FOOT:
		...
		EMIT_SOUND_DYN(edict(), CHAN_BODY, pFootSounds[RANDOM_LONG(0, ARRAYSIZE(pFootSounds) - 1)], 1.0, ATTN_GARG, 0,
			PITCH_NORM + RANDOM_LONG(-10, 10));
		break;
	...
	case GARG_AE_BREATHE:
		if (!(pev->spawnflags & SF_MONSTER_GAG) || m_MonsterState != MONSTERSTATE_IDLE)
			EMIT_SOUND_DYN(edict(), CHAN_VOICE, pBreatheSounds[RANDOM_LONG(0, ARRAYSIZE(pBreatheSounds) - 1)], 1.0,
...
void CGargantua::StartTask(Task_t* pTask)
{
	...
	case TASK_SOUND_ATTACK:
		if (RANDOM_LONG(0, 100) < 30)
			EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, pAttackSounds[RANDOM_LONG(0, ARRAYSIZE(pAttackSounds) - 1)], 1.0,
...
void CGargantua::RunTask(Task_t* pTask)
{
	...
	case TASK_DIE:
	{
		if (gpGlobals->time > m_flWaitFinished)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				CGib* pGib = GetClassPtr(static_cast<CGib*>(nullptr));
