...
void CHAssassin::Shoot()
{
	...
	EjectBrass(pev->origin + gpGlobals->v_up * 32 + gpGlobals->v_forward * 12, vecShellVelocity, pev->angles.y,
...
void CHAssassin::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	case ASSASSIN_AE_TOSS1:
		{
			const Vector vecGunPosition = pev->origin + gpGlobals->v_forward * 34 + Vector(0, 0, 32);
			UTIL_MakeVectors(pev->angles);
...
BOOL CHAssassin::CheckMeleeAttack1(float flDot, float flDist)
{
	if (m_flNextJump < gpGlobals->time && (flDist <= 128 || HasMemory(bits_MEMORY_BADJUMP)) && m_hEnemy != NULL)
	{
		...
		UTIL_TraceHull(pev->origin + Vector(0, 0, 36), vecDest + Vector(0, 0, 36), dont_ignore_monsters, human_hull,
		               ENT(pev), &tr);
...
{
	if (!HasConditions(bits_COND_ENEMY_OCCLUDED) && flDist > 64 && flDist <= 2048 /* && flDot >= 0.5 */
		/* && NoFriendlyFire() */)
	{
		TraceResult tr;
...
BOOL CHAssassin::CheckRangeAttack2(float flDot, float flDist)
{
	...
	{
		Vector vecToss = VecCheckThrow(pev, GetGunPosition(), m_hEnemy->Center(), flDist, 0.5);
