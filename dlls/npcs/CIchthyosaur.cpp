	...
	case ICHTHYOSAUR_AE_SHAKE_LEFT:
		{
			if (m_hEnemy != NULL && FVisible(m_hEnemy))
			{
				...
				if (m_flEnemyTouched < gpGlobals->time - 0.2 && (m_hEnemy->BodyTarget(pev->origin) - pev->origin).
					Length() > (32 + 16 + 32))
					break;
...
Schedule_t* CIchthyosaur::GetScheduleOfType(int Type)
{
	...
	return CBaseFlyingMonster::GetScheduleOfType(Type);
}
...
#define PROBE_LENGTH 150
	...
	Vector f = DoProbe(start + PROBE_LENGTH * Forward);
	Vector r = DoProbe(start + PROBE_LENGTH / 3 * Forward + Right);
	Vector l = DoProbe(start + PROBE_LENGTH / 3 * Forward - Right);
	Vector u = DoProbe(start + PROBE_LENGTH / 3 * Forward + Up);
	Vector d = DoProbe(start + PROBE_LENGTH / 3 * Forward - Up);
...
Vector CIchthyosaur::DoProbe(const Vector& Probe)
{
	...
	if (bBumpedSomething && (m_hEnemy == NULL || tr.pHit != m_hEnemy->edict()))
	{
		...
		float SteeringForce = m_flightSpeed * (1 - frac) * (DotProduct(WallNormal.Normalize(),
		                                                               m_SaveVelocity.Normalize()));
