...
{
	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, pAlertSounds[RANDOM_LONG(0, ARRAYSIZE(pAlertSounds) - 1)], 1.0,
	               ATTN_NORM * 0.5, 0, PITCH_NORM);
}
...
void CLeech::SwimThink()
{
	...
	else
	{
		...
		pev->speed = UTIL_Approach(-(LEECH_SWIM_SPEED * 0.5), pev->speed,
