...
#include "projectiles/CNihilanthHVR.h"
		...
		if (m_hFriend[i] == NULL)
		{
			if (RANDOM_LONG(0, 1) == 0)
			{
				...
				if (iNode != NO_NODE)
				{
					...
					TraceResult tr;
					UTIL_TraceHull(node.m_vecOrigin + Vector(0, 0, 32), node.m_vecOrigin + Vector(0, 0, 32),
					               dont_ignore_monsters, large_hull, nullptr, &tr);
					if (tr.fStartSolid == 0)
			...
			else
			{
				...
				if (iNode != NO_NODE)
				{
					...
					UTIL_TraceHull(node.m_vecOrigin + Vector(0, 0, 36), node.m_vecOrigin + Vector(0, 0, 36),
					               dont_ignore_monsters, human_hull, nullptr, &tr);
					if (tr.fStartSolid == 0)
...
void CNihilanth::NextActivity()
{
	...
	if ((pev->health < gSkillData.nihilanthHealth / 2 || m_iActiveSpheres < N_SPHERES / 2) && m_hRecharger == NULL &&
		m_iLevel <= 9)
	{
		char szName[128];
...
void CNihilanth::TargetSphere(USE_TYPE useType, float value)
{
	...
	pSphere->pev->velocity = m_vecDesired * RANDOM_FLOAT(50, 100) + Vector(
		RANDOM_FLOAT(-50, 50), RANDOM_FLOAT(-50, 50), RANDOM_FLOAT(-50, 50));
}
...
void CNihilanth::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	case 3: // prayer
		if (m_hEnemy != NULL)
		{
			...
			if (pTrigger != nullptr || pTouch != nullptr)
			{
				...
				Vector vecSrc, vecAngles;
				GetAttachment(2, vecSrc, vecAngles);
				CNihilanthHVR* pEntity = static_cast<CNihilanthHVR*>(Create(
					"nihilanth_energy_ball", vecSrc, pev->angles, edict()));
				pEntity->pev->velocity = pev->origin - vecSrc;
	...
	case 6:
		if (m_hEnemy != NULL)
		{
			Vector vecSrc, vecAngles;
			GetAttachment(2, vecSrc, vecAngles);
			CNihilanthHVR* pEntity = static_cast<CNihilanthHVR*>(Create("nihilanth_energy_ball", vecSrc, pev->angles,
			                                                            edict()));
			pEntity->pev->velocity = pev->origin - vecSrc;
			pEntity->ZapInit(m_hEnemy);
		}
...
// TraceAttack
//=========================================================
void CNihilanth::TraceAttack(entvars_t* pevAttacker, float flDamage, Vector vecDir, TraceResult* ptr,
                             int bitsDamageType)
{
	if (m_irritation == 3)
		m_irritation = 2;
	...
	if (m_irritation != 3)
	{
		...
		UTIL_BloodStream(ptr->vecEndPos, vecBlood, BloodColor(),
