...
void COsprey::FlyThink()
{
	...
	{
		m_pGoalEnt = Instance(FIND_ENTITY_BY_TARGETNAME(nullptr, STRING(pev->target)));
		UpdateGoal();
	}
	...
	if (gpGlobals->time > m_startTime + m_dTime)
	{
		...
		do
		{
			m_pGoalEnt = Instance(FIND_ENTITY_BY_TARGETNAME(nullptr, STRING(m_pGoalEnt->pev->target)));
			loopbreaker--; //LRC
		}
...
void COsprey::Flight()
{
	...
	else
	{
		...
		if (pPlayer)
		{
			...
			{
				m_iPitch = pitch;
				EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, "apache/ap_rotor4.wav", 1.0, 0.15,
				               SND_CHANGE_PITCH | SND_CHANGE_VOL, pitch);
				// ALERT( at_console, "%.0f\n", pitch );
			}
		}
